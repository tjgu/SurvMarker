...
#'
#' @param pc_cutoff FDR cutoff for selecting survival-associated PCs (default 0.05).
...
#' @param scale_pca Logical; whether to scale features in PCA.
#' @param weight_type Character string specifying PC weighting scheme:
#'        "variance" (default) or "outcome".
#'        - variance: w_k ∝ lambda_k
#'        - outcome:  w_k ∝ lambda_k * (-log(p_k))
...
pca_based_weighted_score <- function(
  ...
  compute_pc_weights <- function(eigenvalues, pvals, type = c("variance", "outcome")) {
    ...
    if (type == "variance") {
      ...
      pvals_safe <- pmax(pvals, .Machine$double.xmin)
      if (any(!is.finite(pvals_safe)) || any(pvals_safe <= 0) || any(pvals_safe > 1)) {
        stop("pvals must be finite and in (0,1].")
      }
  ...
  if (verbose) {
    if (length(sig_idx) == 0) {
      ...
      message("Survival PCs (PC-level p-value <= ", pc_cutoff, "): ", length(sig_idx))
