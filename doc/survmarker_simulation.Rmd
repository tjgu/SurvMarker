...
Here, using simulated datasets, we illustrate: (i) selection of survival-associated Principal Components (PCs), (ii) PCA-based feature score generation. (iii) empirical-null calibrated feature scoring, and (iv) visualization utilities for diagnosing survival-associated features.

For real datasets (e.g., RNA-seq or miRNA expression), input matrices should be preprocessed prior to analysis. Raw count data should be normalized using Trimmed Mean of M-values (TMM) normalization and subsequently transformed to log2 scale (e.g., via edgeR, DESeq2) before applying SurvMarker.
